...
void main() { WidgetsFlutterBinding.ensureInitialized(); runApp(const SponsorBabeApp()); }
...
class _SponsorBabePageState extends State<SponsorBabePage> with SingleTickerProviderStateMixin {
  ...
  @override Widget build(BuildContext context) {
    if(error!=null) return Scaffold(backgroundColor: Colors.black, body: Center(child: Text('Error:\n$error', style: const TextStyle(color: Colors.redAccent))));
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: Stack(children: [
          ...
          if(isRec) ...[
            Positioned(top:0,left:0,right:0, child: SafeArea(child: Container(padding: const EdgeInsets.symmetric(horizontal:12,vertical:10), color: Colors.black.withOpacity(0.7), child: RunningText(text: musicController.editableTitle, color: Colors.amber, fontSize: 16)))),
            Positioned(left:12,right:12,bottom:130, child: Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.black.withOpacity(0.6), borderRadius: BorderRadius.circular(14), border: Border.all(color: Colors.amber.withOpacity(0.4))), child: Text(musicController.lyricLines.isNotEmpty?musicController.lyricLines[musicController.currentLyricIndex]:'', textAlign: TextAlign.center, style: const TextStyle(color: Colors.amber, fontSize: 20, fontWeight: FontWeight.w900, shadows: [Shadow(color: Colors.black, blurRadius: 10)])))),
            Positioned(left:0,right:0,bottom:90, child: Container(padding: const EdgeInsets.symmetric(horizontal:12,vertical:10), color: Colors.black.withOpacity(0.7), child: RunningText(text: musicController.editableBottomTitle, color: Colors.white70, fontSize: 14))),
            Positioned(left:0,right:0,bottom:18, child: Center(child: GestureDetector(
              onTap: () async {
                // OPSI 1: STOP -> DIALOG TRIM -> AUTO SHARE
              ...
              child: Container(width:78,height:78, decoration: BoxDecoration(color: Colors.red, shape: BoxShape.circle, border: Border.all(color: Colors.white,width:5), boxShadow: [BoxShadow(color: Colors.red.withOpacity(0.7), blurRadius: 25)]), child: Center(child: Text('${musicController.recordSeconds}s', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15)))),
            ))),
            Positioned(top:56,right:12, child: Container(padding: const EdgeInsets.symmetric(horizontal:12,vertical:6), decoration: BoxDecoration(color: Colors.red, borderRadius: BorderRadius.circular(20)), child: Row(children:[Container(width:8,height:8,decoration: const BoxDecoration(color: Colors.white, shape: BoxShape.circle)), const SizedBox(width:6), Text('${musicController.recordSeconds}s / 60s', style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold))]))),
          ],

          // NORMAL PANEL - 3:1 RATIO
          if(!isRec) DraggableScrollableSheet(
