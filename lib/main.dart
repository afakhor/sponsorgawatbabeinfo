...
class _GlobeLearnPageState extends State<GlobeLearnPage> {
  ...
  Future<void> cekIzin() async {
    try {
      ...
      if (st.isGranted && mounted) setState(() => perm = true);
    } catch (_) {
  ...
  Future<void> pickAudio() async {
    ...
    // TEST BLINK - KOMEN DULU WAVEFORM PREPARE
    ...
    setState(() {
      audioFile = f;
      // TEST BLINK - TOTAL HARDCODE DULU
      total = 180;
      s = 0;
      e = 60;
        ...
        Positioned(
          ...
          child: Column(children: [
            const Text("BABE.INFO",
                style: TextStyle(fontSize: 32, fontWeight: FontWeight.w900, color: Colors.amber, letterSpacing: 2)),
            const Text("HeruWingchun", style: TextStyle(fontSize: 14, color: Colors.white70)),
            const SizedBox(height: 4),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
              decoration: BoxDecoration(color: Colors.amber, borderRadius: BorderRadius.circular(8)),
              child: const Text("GLOBE BABE.INFO - TEXTURE babe_gold.jpg",
            ...
            child: Padding(
              ...
              child: Container(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  // TEST BLINK - WAVEFORM DIKOMEN DULU CEK BLINK
                  // if (audioFile != null)
                  //   RepaintBoundary(
                  ...
                  if (audioFile != null)
                    RangeSlider(
                        ...
                        onChanged: (v) {
                          if (v.end - v.start <= 60) setState(() { s = v.start; e = v.end; });
                        }),
                  Row(children: [
                    Expanded(
                        child: ElevatedButton.icon(
                            ...
                            label: Text(audioFile == null ? "AMBIL MUSIK" : "GANTI",
                  ...
                  if (outVideo != null)
                    ElevatedButton.icon(
                        onPressed: () => Share.shareXFiles([XFile(outVideo!.path)],
                            text: "BABE.INFO Globe three_js: https://afakhor.github.io/sponsorgawatbabeinfo/"),
                        icon: const Icon(Icons.share),
                        label: const Text("SHARE WA", style: TextStyle(fontSize: 11)),
