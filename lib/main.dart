...
class _GlobePageState extends State<GlobePage>{
  ...
  String status="Inisialisasi..."; String runText="BABE.INFO HERU WINGCHUN ✨";
  ...
  @override void initState(){
    ...
    threeJs=three.ThreeJS(
      setup: () {},
      onSetupComplete:(){ debugPrint("onSetupComplete OK"); setup(); },
      settings: three.Settings(renderOptions:{"antialias":false,"alpha":true})
    );
    izin();
    // FORCE 2x biar anti always loading
    Future.delayed(Duration(seconds:2),(){ if(!gpuSiap && mounted) setup(); });
    Future.delayed(Duration(seconds:5),(){ if(!gpuSiap && mounted) setup(); });
  }
  ...
  void loadHistory() async{ try{ var d=await getTemporaryDirectory(); var files=d.listSync().where((f)=>f.path.endsWith(".mp4")).map((e)=>e.path).toList(); setState(()=>history=files.reversed.take(20).toList()); }catch(_){} }

  Future<void> setup() async{
    if(gpuSiap) return;
    if(mounted) setState(()=>status="GPU napas 3 detik...");
    ...
    try{
      ...
      rotTimer=Timer.periodic(Duration(milliseconds:16),(_){
        if(globe!=null){ globe!.rotation.y+=0.005; }
        for(var r in rings) r.rotation.z+=0.002;
        if(cube!=null) cube!.rotation.y+=0.008;
      });
      gpuSiap=true;
      if(mounted) setState((){ inited=true; status="GLOBE OK - Full fitur aktif"; });
      debugPrint("SETUP DONE inited=true");
    }catch(e){ debugPrint("setup ERROR $e"); if(mounted) setState(()=>status="Error setup $e - retry..."); Future.delayed(Duration(seconds:2),(){ gpuSiap=false; setup(); }); }
  }

  // FULL FITUR: POLOS DULU, TEXTURE NYUSUL - ANTI LOADING!
  void buildModel(){
    if(threeJs.scene==null){ debugPrint("buildModel scene null"); return; }
    try{
      ...
      if(modelIdx==0){
        ...
        var glowGeo=three.SphereGeometry(1.05,22,22);
        var glowMat=three.MeshBasicMaterial(); glowMat.color=three.Color(t.accent.toDouble()); glowMat.transparent=true; glowMat.opacity=0.12;
        glow=three.Mesh(glowGeo,glowMat); glow!.position.y=0.1; threeJs.scene.add(glow!);
        var torusGeo=three.TorusGeometry(1.0,0.018,10,60);
        ...
        // TEXTURE OPSIONAL - GAK BIKIN LOADING KALAU GAGAL!
        loadTexOptional();
      ...
      else { var geo=three.SphereGeometry(0.9,28,28); var mat=three.MeshPhongMaterial(); mat.color=three.Color(t.globe.toDouble()); globe=three.Mesh(geo,mat); globe!.position.y=0.1; threeJs.scene.add(globe!); }
    }catch(e){ debugPrint("buildModel err $e"); }
  }

  Future<void> loadTexOptional() async{
    try{
      await Future.delayed(Duration(milliseconds:800));
      var loader=three.TextureLoader();
      three.Texture? tex;
      if(customTexPath!=null){
        tex=await loader.fromAsset(customTexPath!);
      } else {
        try{ tex=await loader.fromAsset('assets/images/babe_gold.jpg'); }catch(_){ debugPrint("babe_gold.jpg tidak ada - pakai polos"); return; }
      }
      if(tex!=null && globe!=null){ var m = globe!.material as three.MeshPhongMaterial; m.map=tex; m.needsUpdate=true; if(mounted) setState(()=>status="Skin texture OK"); }
    }catch(e){ debugPrint("loadTexOptional fail $e - tetap polos"); }
  }

  Future<void> pickCustomTex() async{ var r=await FilePicker.platform.pickFiles(type: FileType.image); if(r==null) return; customTexPath=r.files.single.path; await loadTexOptional(); setState(()=>status="Skin custom OK"); }
  ...
  @override Widget build(BuildContext context){
    ...
    return Scaffold(backgroundColor:Color(th.bg),body:Stack(children:[
      ...
      Positioned(top:MediaQuery.of(context).padding.top+8,left:8,right:8,child:Container(padding:EdgeInsets.all(8),decoration:BoxDecoration(color:Colors.black87,borderRadius:BorderRadius.circular(10),border:Border.all(color:Color(th.accent))),child:Column(children:[
        Row(children:[Icon(Icons.public,color:Color(th.accent),size:16),SizedBox(width:6),Expanded(child:Text("${th.name} • ${['Globe','Cube','Ring','Logo'][modelIdx]}",style:TextStyle(fontSize:11,fontWeight:FontWeight.w900,color:Color(th.accent)))),Container(padding:EdgeInsets.symmetric(horizontal:6,vertical:2),decoration:BoxDecoration(color:inited?Colors.green:Colors.orange,borderRadius:BorderRadius.circular(10)),child:Text(inited?"GLOBE OK":"LOADING...",style:TextStyle(fontSize:8,color:Colors.white)))]),
