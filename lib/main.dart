...
class _GlobePageState extends State<GlobePage> with SingleTickerProviderStateMixin{
  ...
  Future<void> bustVideo() async{
    ...
    try{
      ...
      var se=await FFmpegKit.execute(cmd); if(ReturnCode.isSuccess(await se.getReturnCode())){ setState((){outVideo=File(out); load=false; status="VIDEO JADI!";}); } else { setState((){load=false; status="Gagal";}); }
    }catch(e){ setState((){load=false; status="Error $e";}); }
  }

  @override Widget build(BuildContext context){
    var th=themes[temaIdx];
    Widget bgW = bgFile!=null? Image.file(bgFile!,fit:BoxFit.cover,width:double.infinity,height:double.infinity) : Image.asset('assets/images/bg.jpg',fit:BoxFit.cover,width:double.infinity,height:double.infinity,errorBuilder:(_,__,___)=>Container(decoration:BoxDecoration(gradient:LinearGradient(colors:[Colors.black,Color(0xFF1A1A00)],begin:Alignment.topCenter,end:Alignment.bottomCenter))));
    return Scaffold(backgroundColor: Colors.black, body: Stack(children:[
      Positioned.fill(child: bgW),
      Positioned(top:MediaQuery.of(context).padding.top+8,left:10,right:10,child:Container(padding:EdgeInsets.all(10),decoration:BoxDecoration(color:Colors.black87,borderRadius:BorderRadius.circular(12),border:Border.all(color:Color(th.accent))),child:Row(children:[Icon(Icons.public,color:Color(th.accent)),SizedBox(width:8),Expanded(child:Text("REAL 3D MATH • ${th.name} • Android & Web Sama!",style:TextStyle(color:Color(th.accent),fontWeight:FontWeight.bold,fontSize:11))),Container(padding:EdgeInsets.symmetric(horizontal:8,vertical:4),decoration:BoxDecoration(color:Colors.green,borderRadius:BorderRadius.circular(20)),child:Text("3D OK",style:TextStyle(fontSize:9,color:Colors.white)))]))),
      Positioned(top:120,left:0,right:0,child:Center(child:Column(children:[
        RepaintBoundary(key:globeKey, child:GestureDetector(
          ...
          child:Transform(alignment:Alignment.center,transform: Matrix4.identity()..scale(scale),child:Container(width:300,height:300,child:CustomPaint(size:Size(300,300), painter:Real3DGlobePainter(rotX, rotY, th.accent)))),
        )),
        SizedBox(height:12), Text("ANDROID & WEB REAL 3D - Geser Kiri Kanan Atas Bawah",style:TextStyle(color:Color(th.accent),fontSize:10,fontWeight:FontWeight.bold)),
      ...
      SafeArea(child:Align(alignment:Alignment.bottomCenter,child:Container(margin:EdgeInsets.all(10),padding:EdgeInsets.all(12),decoration:BoxDecoration(color:Colors.black.withOpacity(0.88),borderRadius:BorderRadius.circular(18),border:Border.all(color:Color(th.accent))),child:Column(mainAxisSize:MainAxisSize.min,children:[
        ...
        SizedBox(height:8), Row(children:[
          ...
          SizedBox(width:8), Expanded(flex:2,child:ElevatedButton(onPressed:load?null:bustVideo,child:Text(load?"RENDER...":"BUST VIDEO + GLOBE"),style:ElevatedButton.styleFrom(backgroundColor:Colors.greenAccent,foregroundColor:Colors.black))),
        ]),
...
class Real3DGlobePainter extends CustomPainter{
  final double rotX, rotY; final int accent;
  Real3DGlobePainter(this.rotX,this.rotY,this.accent);
  List<double> rotate3D(double x,double y,double z){
    double cosY=math.cos(rotY); double sinY=math.sin(rotY);
    double x1=x*cosY - z*sinY; double z1=x*sinY + z*cosY;
    double cosX=math.cos(rotX); double sinX=math.sin(rotX);
    double y1=y*cosX - z1*sinX; double z2=y*sinX + z1*cosX;
    return [x1,y1,z2];
  }
  @override void paint(Canvas canvas, Size size){
    var center=Offset(size.width/2,size.height/2); double radius=size.width*0.42;
    canvas.drawCircle(center, radius+4, Paint()..color=Colors.black);
    for(double r=radius; r>0; r-=1.2){ double t=r/radius; double shade=0.3+0.7*(1-t*0.7); var p=Paint()..color=Color.lerp(Color(0xFF8B6914), Color(0xFFFFD700), shade)!..style=PaintingStyle.stroke..strokeWidth=1.2; canvas.drawCircle(center, r, p); }
    var gridPaint=Paint()..color=Color(accent).withOpacity(0.28)..style=PaintingStyle.stroke..strokeWidth=0.8;
    for(int lon=0; lon<12; lon++){ double lonRad=lon*math.pi/6; var path=Path(); bool first=true; for(double lat=-math.pi/2; lat<=math.pi/2; lat+=0.06){ double x=radius*math.cos(lat)*math.cos(lonRad); double y=radius*math.sin(lat); double z=radius*math.cos(lat)*math.sin(lonRad); var rot=rotate3D(x,y,z); double persp=320/(320+rot[2]); double sx=center.dx+rot[0]*persp; double sy=center.dy+rot[1]*persp; if(rot[2]>-radius*0.3){ if(first){ path.moveTo(sx,sy); first=false; } else { path.lineTo(sx,sy); } } else { first=true; } } canvas.drawPath(path, gridPaint); }
    for(int latIdx=-4; latIdx<=4; latIdx++){ if(latIdx==0) continue; double lat=latIdx*math.pi/8; var path=Path(); bool first=true; for(double lon=0; lon<=2*math.pi; lon+=0.06){ double x=radius*math.cos(lat)*math.cos(lon); double y=radius*math.sin(lat); double z=radius*math.cos(lat)*math.sin(lon); var rot=rotate3D(x,y,z); double persp=320/(320+rot[2]); double sx=center.dx+rot[0]*persp; double sy=center.dy+rot[1]*persp; if(rot[2]>-radius*0.2){ if(first){ path.moveTo(sx,sy); first=false; } else { path.lineTo(sx,sy); } } else { first=true; } } canvas.drawPath(path, gridPaint); }
    var textStyle=TextStyle(color:Colors.black.withOpacity(0.88),fontSize:13,fontWeight:FontWeight.w900);
    for(int i=0;i<5;i++){ double lat=(i-2)*0.45; for(int j=0;j<8;j++){ double lon=j*math.pi/4+rotY*0.6; double x=radius*0.82*math.cos(lat)*math.cos(lon); double y=radius*0.82*math.sin(lat); double z=radius*0.82*math.cos(lat)*math.sin(lon); var rot=rotate3D(x,y,z); if(rot[2]>0){ double persp=320/(320+rot[2]); double sx=center.dx+rot[0]*persp; double sy=center.dy+rot[1]*persp; double sc=persp; if(sx>20&&sx<size.width-20){ var tp=TextPainter(text:TextSpan(text:"BABE.INFO",style:textStyle.copyWith(fontSize:13*sc)),textDirection:TextDirection.ltr)..layout(); canvas.save(); canvas.translate(sx,sy); canvas.scale(sc); tp.paint(canvas, Offset(-tp.width/2,-tp.height/2)); canvas.restore(); } } } }
    var rootPaint=Paint()..color=Colors.black.withOpacity(0.7)..style=PaintingStyle.stroke..strokeWidth=1.5;
    var rnd=math.Random(123); for(int i=0;i<10;i++){ double lat=(rnd.nextDouble()-0.5)*math.pi; double lon=rnd.nextDouble()*2*math.pi; var path=Path(); bool first=true; double clat=lat, clon=lon; for(int k=0;k<12;k++){ double x=radius*math.cos(clat)*math.cos(clon); double y=radius*math.sin(clat); double z=radius*math.cos(clat)*math.sin(clon); var rot=rotate3D(x,y,z); if(rot[2]>-radius*0.1){ double persp=320/(320+rot[2]); double sx=center.dx+rot[0]*persp; double sy=center.dy+rot[1]*persp; if(first){ path.moveTo(sx,sy); first=false; } else { path.lineTo(sx,sy); } } clat+=(rnd.nextDouble()-0.5)*0.2; clon+=(rnd.nextDouble()-0.5)*0.2; } canvas.drawPath(path, rootPaint); }
    var gloss=Paint()..shader=RadialGradient(colors:[Colors.white.withOpacity(0.45),Colors.transparent],center:Alignment(-0.35,-0.35),radius:0.75).createShader(Rect.fromCircle(center:center,radius:radius)); canvas.drawCircle(center, radius, gloss);
    canvas.drawCircle(center, radius, Paint()..color=Color(accent)..style=PaintingStyle.stroke..strokeWidth=2.5);
    canvas.drawCircle(center, radius, Paint()..color=Color(accent).withOpacity(0.35)..style=PaintingStyle.stroke..strokeWidth=12..maskFilter=MaskFilter.blur(BlurStyle.normal,12));
  }
